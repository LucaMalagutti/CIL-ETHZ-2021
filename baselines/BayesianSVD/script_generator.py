        ...
        with open(script_file, "w") as outf:
            ...
            for split in ["90aug.train", "100aug.train", "aug.valid", "aug.submission"]:
                for block in ["user", "item"]:
                    ...
                    file_str += "/data_features/bagofitemsusers/CIL_data"
                    file_str += split
                    file_str += ".libfmrel_" + block
                    command_str += file_str + ".libfm"
                    command_str += " -ofilex " + file_str + ".x"
                    command_str += " -ofiley " + BayesianSVD_dir + "/data_features/bagofitemsusers/totrash.y"
            ...
            outf.write("cd " + BayesianSVD_dir + "/data_features/bagofitemsusers\n")
            for file in os.listdir(BayesianSVD_dir + "/data_features/bagofitemsusers"):
                if file[-6:] == ".libfm":
                    outf.write("rm " + file + "\n")
