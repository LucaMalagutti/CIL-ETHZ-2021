...
class fm_learn_mcmc : public fm_learn {
 public:
  ...
  // (alpha, lambda) deterministically define sigma
 ...
 protected:
  ...
  void draw_w0(double& w0, double& reg, Data& train, double T);
  void draw_w(double& w, double& w_mu, double& w_lambda, sparse_row<DATA_FLOAT>& feature_data, double T);
  void draw_w_rel(double& w, double& w_mu, double& w_lambda, sparse_row<DATA_FLOAT>& feature_data, relation_cache* r_cache, double T);
  void draw_v(double& v, double& v_mu, double& v_lambda, sparse_row<DATA_FLOAT>& feature_data, double T);
  void draw_v_rel(double& v, double& v_mu, double& v_lambda, sparse_row<DATA_FLOAT>& feature_data, relation_cache* r_cache, double T);
...
void fm_learn_mcmc::draw_all(Data& train, double T, int i) {
  ...
  if (fm->k1) {
    ...
    for (uint i = 0; i < train.data_t->getNumRows(); i++) {
      ...
      draw_w(fm->w(row_index), w_mu(g), w_lambda(g), *feature_data, T);
    }
    ...
    for (uint r = 0; r < train.relation.dim; r++) {
      ...
      for (uint i = 0; i < join.data->data_t->getNumRows(); i++) {
        ...
        draw_w_rel(fm->w(row_index+join.data->attr_offset), w_mu(g), w_lambda(g), *feature_data, r_cache, T);
      }
  ...
  for (int f = 0; f < fm->num_factor; f++) {
    ...
    for (uint i = 0; i < train.data_t->getNumRows(); i++) {
      ...
      uint g = meta->attr_group(row_index);
      draw_v(v[row_index], v_mu(g,f), v_lambda(g,f), *feature_data, T);
    }
    ...
    for (uint r = 0; r < train.relation.dim; r++) {
      ...
      for (uint i = 0; i < join.data->data_t->getNumRows(); i++) {
        ...
        uint g = meta->attr_group(row_index+join.data->attr_offset);
        draw_v_rel(v[row_index+join.data->attr_offset], v_mu(g,f), v_lambda(g,f), *feature_data, r_cache, T);
      }
...
void fm_learn_mcmc::draw_w0(double& w0, double& reg, Data& train, double T) {
  // h = 1
  // h^2 = 1
  // \sum e*h = \sum e
  // \sum h^2 = \sum 1
  double w0_sigma_sqr, w0_sigma_sqr_old;
  double w0_mean = 0;
...
void fm_learn_mcmc::draw_w(double& w, double& w_mu, double& w_lambda, sparse_row<DATA_FLOAT>& feature_data, double T) {
  double w_sigma_sqr = 0;
  double w_sigma_sqr_old;
  double w_mean = 0;
  for (uint i_fd = 0; i_fd < feature_data.size; i_fd++) {
    uint& train_case_index = feature_data.data[i_fd].id;
    FM_FLOAT x_li = feature_data.data[i_fd].value;
...
void fm_learn_mcmc::draw_w_rel(double& w, double& w_mu, double& w_lambda, sparse_row<DATA_FLOAT>& feature_data, relation_cache* r_cache, double T) {
  double w_sigma_sqr = 0;
  double w_sigma_sqr_old;
  double w_mean = 0;
  // w_sigma_sqr = \sum h^2
...
void fm_learn_mcmc::draw_v(double& v, double& v_mu, double& v_lambda, sparse_row<DATA_FLOAT>& feature_data, double T) {
  double v_sigma_sqr = 0;
  double v_sigma_sqr_old;
  double v_mean = 0;
...
void fm_learn_mcmc::draw_v_rel(double& v, double& v_mu, double& v_lambda, sparse_row<DATA_FLOAT>& feature_data, relation_cache* r_cache, double T) {
  double v_sigma_sqr = 0;
  double v_sigma_sqr_old;
  double v_mean = 0;
  // v_sigma_sqr = \sum h^2
