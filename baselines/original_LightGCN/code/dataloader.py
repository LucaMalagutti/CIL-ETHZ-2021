...
class CIL(BasicDataset):
    def __init__(self, config=world.config, path="../data/cil"):
        ...
        self.train_unique_users = np.array(train_df['user'].unique())
        test_df = pd.read_csv(test_file)

        self.test_unique_users = np.array(test_df['user'].unique())
        self.train_data_size = train_df.shape[0]
        self.test_data_size = test_df.shape[0]

        self.user_item_net = sp.load_npz("../data/cil/R_train.npz")
        self.test_user_item_net = sp.load_npz("../data/cil/R_test.npz")
        ...
        self.test_dict = self._build_test(self.test_unique_users)
    ...
    def get_sparse_graph(self):
        ...
        if self.graph is None:
            try:
                ...
                A_mask = A_mask.tolil()
                R_mask = sp.load_npz(self.path + "/R_mask_train.npz")
                A_mask[: self.n_user, self.n_user:] = R_mask
                A_mask[self.n_user:, : self.n_user] = R_mask.T
                ...
                print(f"Generating A_tilde took {end - s}s...")
...
class Loader(BasicDataset):
    ...
    def __init__(self, config=world.config, path="../data/gowalla"):
        ...
        self.path = path
        train_unique_users, trainItem, trainUser = [], [], []
        test_unique_users, testItem, testUser = [], [], []
